...
def Enumerate_Tcodes(self, app):
    value = 0.0
    self.progressbar.set(value)
    self.textboxtk.delete("1.0", "end")
    Module = """Module - Enumerate for Accessible T-Codes\n-------------------------"""
    ...
    if(self.session is None):
        ...
        # Check if user had inputed path with custom T-Codes in json file
        if(self.entry.get()):
            TcodeListFilePath = self.entry.get()
            self.textboxtk.insert("insert", '\u2713 '+"Path set for referencing T-Codes list is " + TcodeListFilePath + "\n", 'ycolor')
            # load JSON from file
            with open(TcodeListFilePath) as f:
                TLJsonData = json.load(f)
            self.textboxtk.insert("insert", "\n", 'ycolor')
            self.textboxtk.insert("insert", "Fetching Accessible T-Codes.... \n", 'ycolor')
            value = value + 0.1 #3
            self.progressbar.set(value)
            app.update_idletasks()
        else:
            ...
            filename = 'T-Codes_List_Small.json'
            file_path = os.path.join(path, filename)
            self.textboxtk.insert("insert", '\u2717 '+"No T-Codes list reference file provided for enumeration, only default T-Codes in ../Config/T-Codes_List_Small.json is used for enumeration \n", 'ycolor')
        ...
        for index, (key, value) in enumerate(TLJsonData.items()):
            ...
            self.session.findById("wnd[0]/tbar[0]/okcd").text = key
